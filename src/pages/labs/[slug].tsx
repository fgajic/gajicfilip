...
export default function LabPage({success, post}: InferGetStaticPropsType<typeof getStaticProps>) {
    return <PostPage post={post} success={success}/>
}

export async function getStaticPaths() {
    const posts = await PostSearcher.search("", PostType.Lab);
...
export async function getStaticProps(ctx: GetStaticPropsContext<{ slug: string }>) {
    ...
    const result = await getPost(`${slug}.mdx`, PostType.Lab);

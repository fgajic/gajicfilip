...
Part A — Multi-Site Nginx with Local Name Resolution
...
1) Ensure a non-root sudo user and SSH key auth:

**Explanation:** SSH keys are more secure than passwords because they use cryptography instead of text that can be guessed or stolen. We'll generate a key pair (public and private) and use the public key to authenticate.
```bash
id $USER
mkdir -p ~/.ssh && chmod 700 ~/.ssh
test -f ~/.ssh/id_ed25519 || ssh-keygen -t ed25519 -C "$USER@$(hostname)" -N "" -f ~/.ssh/id_ed25519
cat ~/.ssh/id_ed25519.pub | sudo tee -a /home/$USER/.ssh/authorized_keys >/dev/null
...
```bash
ssh -p 2222 $USER@$(hostname -I | awk '{print $1}') true && echo "✓ SSH hardened works" || echo "✗ SSH test failed"
```
...
Part D — Service Minimization and Nginx Hardening
...
1) Disable unnecessary services (examples):

**Explanation:** Each running service uses system resources and could potentially be exploited. We'll identify and disable services that aren't needed for our web server, reducing the "attack surface" of our system.
```bash
systemctl list-unit-files --type=service | grep enabled | head -20
# Decide and disable non-required services, e.g.:
...
**Explanation:** Compression reduces file sizes (making pages load faster), and caching tells browsers to store static files locally instead of downloading them repeatedly. This improves performance and reduces server load.
```bash
sudo bash -c 'cat >> /etc/nginx/sites-available/site1.conf << "EOF"'
...
```bash
nginx -T | grep -E "(server_tokens|add_header)" | head -10
...
Part F — Reverse Proxy and Backend Integration

**What you'll learn:** A reverse proxy sits between clients and backend services, forwarding requests and responses. This is how modern web applications work - Nginx serves static files and forwards dynamic requests to application servers. This is also a key security benefit: you only need to expose ports 22, 80, and 443 to the internet, while backend services can run on internal ports (like 3000, 8000, etc.) that are never directly accessible from outside.

1) Create a simple backend service:

**Explanation:** We'll create a simple Node.js web server that runs on port 3000. This simulates a real application server that handles dynamic content (like user accounts, databases, etc.) while Nginx handles static files.
```bash
...
**Explanation:** We'll configure Nginx to act as a reverse proxy. When someone visits `api.local`, Nginx will forward their request to our Node.js server running on port 3000, then send the response back to the client. This hides the backend server from direct access. Notice how port 3000 is never exposed to the internet - only Nginx (port 80) is accessible, and it internally forwards requests to the backend.
```bash
...
```bash
# Create a second backend service on port 3001
...
node ~/backend-app/server2.js &
BACKEND2_PID=$!
echo "Backend 2 PID: $BACKEND2_PID"

# Create Nginx config for the second service
sudo bash -c 'cat > /etc/nginx/sites-available/service2.conf << "EOF"'
upstream backend2 {
...
# Test both services
echo "Testing api.local (port 3000):"
curl http://api.local/ | jq . || curl http://api.local/
echo "Testing service2.local (port 3001):"
curl http://service2.local/ | jq . || curl http://service2.local/

# Show that ports 3000 and 3001 are not accessible from outside
echo "Ports 3000 and 3001 are not accessible externally - only through Nginx!"
```
...
Part G — Rate Limiting and Security
...
Part H — Health Checks and Backend Monitoring
...
Part I — Deployment, Rollback, and Backups
...
- Demonstration that backend ports (3000, 3001) are not externally accessible
...
Grading Rubric
- Nginx multi-site and permissions: 15%
- SSH hardening (port, root, passwords): 15%
- Firewall + Fail2Ban correctness and tests: 10%
- Nginx hardening and reverse proxy: 15%
- Rate limiting configuration and testing: 15%
- Health checks and backend monitoring: 15%
- Monitoring/logs and rotation: 10%
- Deployment + backups: 5%

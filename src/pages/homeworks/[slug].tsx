...
export default function HomeworkPage({success, post}: InferGetStaticPropsType<typeof getStaticProps>) {
    return <PostPage post={post} success={success}/>
}

export async function getStaticPaths() {
    const posts = await PostSearcher.search("", PostType.Homework);
...
export async function getStaticProps(ctx: GetStaticPropsContext<{ slug: string }>) {
    ...
    const result = await getPost(`${slug}.mdx`, PostType.Homework);
